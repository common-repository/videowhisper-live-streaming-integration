// VideoWhisper.com WebRTC implemetation / 2021.07.17
let startTime;
...
var hasGotDescription = false;
var hasCandidate      = false;
var hasDescription    = null;
var hasServerPlay     = false;
...
function wsConnect(url)
{
	...
	wsConnection.onopen = function()
	{
		...
		peerConnection.addEventListener( 'signalingstatechange', e => onSignalingStateChange() );
		peerConnection.addEventListener( 'iceconnectionstatechange', e => onIceStateChange( peerConnection, e ) );
		peerConnection.addEventListener( 'connectionstatechange', e => onConnectionStateChange( peerConnection, e ) );

		peerConnection.addEventListener( 'icegatheringstatechange', e => onIceGatheringState( e ) );
		...
		console.log( "wsConnect: " + wsURL,  peerConnection );

		sendPlayGetOffer();

	}

	function onIceGatheringState()
	{

		if (peerConnection) {
			console.log( 'onIceGatheringState', peerConnection.iceGatheringState, peerConnection );
			jQuery( "#sdpDataTag" ).html( 'WebRTC: ' + peerConnection.connectionState + ' ' + peerConnection.iceConnectionState + ' ' + peerConnection.iceGatheringState + ' ' + peerConnection.signalingState );

			if (peerConnection.iceGatheringState == 'gathering') {
				hasCandidate = true;
			}
			if (hasGotDescription) {
				serverPlay();
			}
		}
	}
	...
	function onSignalingStateChange()  	{
		let state;
		if (peerConnection) {
			console.log( 'onSignalingStateChange', peerConnection.signalingState, hasGotDescription, hasCandidate, peerConnection );

			jQuery( "#sdpDataTag" ).html( 'WebRTC: ' + peerConnection.connectionState + ' ' + peerConnection.iceConnectionState + ' ' + peerConnection.iceGatheringState + ' ' + peerConnection.signalingState );
		}
	}

	function connectionUpdated(connectionState)
	{
		jQuery( "#sdpDataTag" ).html( 'WebRTC: ' + peerConnection.connectionState + ' ' + peerConnection.iceConnectionState + ' ' + peerConnection.iceGatheringState + ' ' + peerConnection.signalingState );
			...
			if ( ! videoPlaying) {
				...
				if (promise !== undefined) {
					promise.catch(
                        ...
                        jQuery( "#sdpDataTag" ).html( '<button class="ui button compact red" onclick="remoteVideo.play(); jQuery(\'#sdpDataTag\').html(\'\')"><i class="play icon"></i> Tap to Play</button> ' + error );
	...
	function onConnectionStateChange(pc, event) {
		...
		if (pc) {
			...
			console.log( `onConnectionStateChange connectionState: ${pc.connectionState}` );
	...
	function onIceStateChange(pc, event) {
		...
		if (pc) {
			...
			console.log( `onIceStateChange iceConnectionState: ${pc.iceConnectionState}` );
			console.log( 'onIceStateChange event: ', event );
	...
	wsConnection.onmessage = function(evt)
	{
		...
		if (msgStatus == 514) { // repeater stream not ready
			...
			if (iceCandidates !== undefined) {
				for (var index in iceCandidates) {
					console.log( 'iceCandidate', index, JSON.stringify( iceCandidates[index] ) );
		...
		if ('sendResponse'.localeCompare( msgCommand ) == 0 && hasServerPlay) {
			console.log( "wsConnection closing", msgCommand );
...
onAddIceCandidateSuccess = (pc) => {
	console.log( 'onAddIceCandidateSuccess', pc, wsConnection );
...
function gotDescription(description)
{
	...
	peerConnection.setLocalDescription(
		...
		function ()
		{
			console.log( 'peerConnection.setLocalDescription', hasCandidate )
...
serverPlay = () =>
{
		...
		console.log( 'serverPlay', hasServerPlay, peerConnection, wsConnection );

	if (hasServerPlay) {
		return;
	}

		wsConnection.send( '{"direction":"play", "command":"sendResponse", "streamInfo":' + JSON.stringify( streamInfo ) + ', "sdp":' + JSON.stringify( hasDescription ) + ', "userData":' + JSON.stringify( userData ) + '}' );
