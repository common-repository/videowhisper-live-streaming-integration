				...
				*************************************************************************/
				_getDisplayTextForRecordField: function (record, fieldName) {
					...
					if (field.type == 'date') {
						...
						var options = this._getOptionsForField(
							fieldName,
							{
								record: record,
								value: fieldValue,
		...
		$.hik.jtable.prototype,
		{
			...
			*************************************************************************/
			_createCheckBoxStateArrayForField: function (fieldName) {
				...
				var currentIndex = 0;
				$.each(
					this.options.fields[fieldName].values,
					function (propName, propValue) {
						if (currentIndex++ < 2) {
			...
			*************************************************************************/
			_createAddRecordDialogDiv: function () {
				...
				if (self.options.addRecordButton) {
					...
					self.options.addRecordButton.click(
						function (e) {
							e.preventDefault();
		...
		$.hik.jtable.prototype,
		{
			...
			*************************************************************************/
			_showUpdateAnimationForRow: function ($tableRow) {
				...
				$tableRow.stop( true, true ).addClass(
					className,
					'slow',
					'',
					function () {
			...
			*************************************************************************/
			_removeRowsFromTableWithAnimation: function ($rows, animationsEnabled) {
				...
				if (animationsEnabled) {
					...
					$rows.stop( true, true ).addClass( className, 'slow', '' ).promise().done(
						function () {
		...
		$.hik.jtable.prototype,
		{
			...
			*************************************************************************/
			_buildDefaultSortingArray: function () {
				...
				$.each(
					self.options.defaultSorting.split( "," ),
					function (orderIndex, orderValue) {
						$.each(
							self.options.fields,
		...
		$.hik.jtable.prototype,
		{
			...
			*************************************************************************/
			_createColumnSelection: function () {
				...
				this._$table.children( 'thead' ).bind(
					'contextmenu',
					function (e) {
			...
			*************************************************************************/
			_makeColumnResizable: function ($columnHeader) {
				...
				.mousedown(
					function (downevent) { // handle mousedown event for the handler
			...
			*************************************************************************/
			_saveColumnSettings: function () {
				var self          = this;
				var fieldSettings = '';
				this._$table.find( '>thead >tr >th.jtable-column-header' ).each(
					function () {
						var $cell           = $( this );
						var fieldName       = $cell.data( 'fieldName' );
			...
			*************************************************************************/
			openChildTable: function ($row, tableOptions, opened) {
				...
				if (self.options.openChildAsAccordion) {
					$row.siblings( '.jtable-data-row' ).each(
						function () {
				...
				self.closeChildTable(
					...
					function () {
						...
						$childTableContainer.hide().slideDown(
							'fast',
							function () {
								if (opened) {
									opened(
